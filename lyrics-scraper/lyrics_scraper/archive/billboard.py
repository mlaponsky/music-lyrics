...
class ChartData:
    ...
    def fetchNeighborDates(self):
        ...
        for link in soup.find_all('a', {'class': 'chart-nav-link'}, href=True):
            if link['title'] == 'Next Week':
                next = link['href'].split('/')[-1]
                if next != '' and next != self.name:
                    self.next = next
            elif link['title'] == 'Previous Week':
                previous = link['href'].split('/')[-1]
                if previous != '' and previous != self.name:
                    self.previous = previous
        return self.previous, self.next

    def getPreviousChart(self):
        if self.previous:
            return ChartData(self.name, date=self.previous)
        return False

    def getNextChart(self):
        if self.next:
            return ChartData(self.name, date=self.next)
        return False
